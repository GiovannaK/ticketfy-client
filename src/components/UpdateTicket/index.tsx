import { CardContent, CircularProgress, FormControlLabel, Grid, MenuItem } from '@mui/material'
import React from 'react'
import { CardCreateEvent, CheckBoxInput, FormButton, FormTitle, InputCreate, Label, StyledTextField, UploadButton, SelectCreate } from './styles'
...
export const UpdateTicketForm = (): JSX.Element => {
  ...
  const postTicket = async(ticket: ITicket) => {
    ...
    return await api.put('/ticket', newTicket)
  }

  const {isLoading: loading, mutate} = useMutation(postTicket, {
    onSuccess: () => {
      toast.info('Ticket atualizado com sucesso!')
    },
    onError: () => {
      toast.error('Não foi possível atualizar ticket.')
    }
  })
